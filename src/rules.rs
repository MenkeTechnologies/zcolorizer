...
mod tests {
    ...
    #[test]
    fn whole_match_without_token_falls_back_to_default() {
        // A plain `RuleDef::new` (no token) paints its match with the `default` token.
        let rule = Rule::compile(&RuleDef::new("w", r"\w+")).unwrap();
        assert!(!rule.has_named_groups);
        assert_eq!(rule.whole_token, tokens::DEFAULT);
    }

    #[test]
    fn ignore_case_compiles_and_matches_any_case() {
        let rule = Rule::compile(&RuleDef::with_token("e", r"error", "error").ci()).unwrap();
        assert!(rule.regex.is_match("ERROR"));
        assert!(rule.regex.is_match("Error"));
        assert!(rule.regex.is_match("error"));
        // `.ci()` flips the flag on the def too.
        assert!(RuleDef::new("x", "y").ci().ignore_case);
    }
    ...
    #[test]
    fn bad_regex_surfaces_error_with_rule_name() {
        let err = Rule::compile(&RuleDef::new("oops", r"(unclosed")).unwrap_err();
        match err {
            crate::error::Error::BadRule { name, .. } => assert_eq!(name, "oops"),
            other => panic!("expected BadRule, got {other:?}"),
        }
    }

    #[test]
    fn compile_all_fails_on_first_bad_rule() {
        let defs = vec![
            RuleDef::new("good", r"\d+"),
            RuleDef::new("bad", r"[unterminated"),
        ];
        let err = compile_all(&defs).unwrap_err();
        assert!(matches!(err, crate::error::Error::BadRule { name, .. } if name == "bad"));
    }

    #[test]
    fn unnamed_groups_are_not_named_group_rules() {
        // A regex with only *unnamed* capture groups is a whole-match rule.
        let rule = Rule::compile(&RuleDef::new("g", r"(\d+)-(\d+)")).unwrap();
        assert!(!rule.has_named_groups);
        assert_eq!(rule.whole_token, tokens::DEFAULT);
    }

    #[test]
    fn group_tokens_index_zero_is_implicit_whole_match() {
        // capture_names()'s first entry is the whole match and is always None.
